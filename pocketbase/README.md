...
> ## ⚠️ Совместимость версий (важно)
> Хуки `pb_hooks/wyrm.pb.js` и миграция написаны под **PocketBase server v0.22.x**
> (JSVM API до 0.23: `onRecord*Request`, `$app.dao()`, без `e.next()`). SDK в
> `package.json` закреплён на `pocketbase@^0.21.5` (= `<0.22`), совместимом с
> сервером 0.22.x.
> **Не обновляйте сервер до ≥0.23 без переписывания хуков** — там хуки стали
> `onRecord*Success`, DAO-методы переехали на `$app`, и каждый хендлер обязан
> заканчиваться `e.next()`. Иначе серверная логика (канон, защита полей,
> уведомления, rate-limit) молча перестанет работать. Шапка файла хука об этом
> предупреждает.

> ## SMTP (подтверждение почты и сброс пароля)
> Включается в админке: **Settings → Mail settings** — укажите SMTP-хост, порт,
> логин/пароль (напр. Yandex 360 / VK / любой РФ-SMTP). После этого работают
> клиентские `store.requestPasswordReset()` / `requestVerification()` и письма
> верификации. Без SMTP вход остаётся по паролю, а кнопки восстановления покажут
> сообщение «почта не настроена на сервере».

> ## OAuth (VK / Yandex)
> Включается в админке: **Settings → Auth providers**. Для Yandex/VK добавьте
> client id/secret и redirect URL. Клиент уже вызывает `pb.authWithOAuth2`
> (`store.signInOAuth('vk'|'yandex'|…)`); кнопки появляются автоматически. Без
> настройки провайдера кнопка покажет «провайдер не настроен».
