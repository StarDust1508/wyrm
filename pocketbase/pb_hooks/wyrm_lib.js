...
// On OAuth signup PocketBase fills email/name from the provider but NOT our
// required `handle` (unique) / `role`. Derive a unique handle from the email
// local-part (or name) and fill name if the provider gave none.
function ensureHandle(record, dao, oauthUser) {
  if (!record.getString("handle")) {
    const email = record.getString("email") || (oauthUser && oauthUser.email) || "";
    const nm = record.getString("name") || (oauthUser && oauthUser.name) || "";
    let base = (email.split("@")[0] || nm || "author")
    ...
    for (let i = 1; i <= 60; i++) {
      let taken = false;
      try {
        const rows = dao.findRecordsByFilter("users", "handle = {:h}", "", 1, 0, { h: cand });
        taken = rows && rows.length > 0;
      } catch (_) { taken = false; }
      if (!taken) break;
      cand = base + i;
    }
    record.set("handle", cand);
  }
  if (!record.getString("name")) {
    const em = record.getString("email") || (oauthUser && oauthUser.email) || "";
    const fromEmail = (em.split("@")[0] || "").trim();
    record.set("name", (oauthUser && oauthUser.name) || fromEmail || record.getString("handle") || "Автор");
  }
}

// ----------------------------------------------------------------------------
// Exports — required from each handler in wyrm.pb.js.
// PocketBase v0.22 JSVM runs every hook handler in an ISOLATED runtime, so
// top-level helpers in a *.pb.js file are NOT visible inside handlers. The
// handlers `require()` this (non-".pb.js", so NOT auto-loaded) module instead.
// Internal helpers call each other by name within this single module scope, so
// only the handler-facing functions need to be exported.
// ----------------------------------------------------------------------------
module.exports = {
  logErr,
  isAdminRequest,
